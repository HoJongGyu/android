...
import com.hjk.music_3.databinding.ActivityYoutubeaddBinding;
...
public class YouTubeAddActivity extends AppCompatActivity {
    Music music=new Music();
    YouTube youtube=new YouTube();

    ActivityYoutubeaddBinding binding;
    Gson gson=new Gson();

    MusicService musicService;
    YouTubeService youtubeService;

    private static final String TAG = YouTubeAddActivity.class.getSimpleName();
    ...
    public void onCreate(Bundle save){
        super.onCreate(save);
        binding= DataBindingUtil.setContentView(this, R.layout.activity_youtubeadd);
...
//                youtube_extract(str);
    ...
    public void youtube(String result){
        ...
        youtube.setMp4(result);
        youtube.setTitle(binding.title.getText().toString());
        youtube.setImage(image);
        youTubeViewModel.insert(youtube);
        ...
        youtube.enqueue(new Callback<ResponseBody>() {
            ...
            public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                try {
                    ...
                    System.out.println(result);
                    if (result.equals("1")) {
                        Toast.makeText(getApplicationContext(), "유튜브 추가 완료", Toast.LENGTH_SHORT).show();
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                }
            ...
            @Override
            public void onFailure(Call<ResponseBody> call, Throwable t) {
                Toast.makeText(getApplicationContext(), "유튜브 추가 실패", Toast.LENGTH_SHORT).show();
